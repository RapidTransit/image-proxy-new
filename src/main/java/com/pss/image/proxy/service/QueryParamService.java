...
public class QueryParamService {
    ...
    public String extractQueryParam(HttpServerRequest request, String path) {
        ...
        if (profile == null || profile.isEmpty()) {
            log.warn("`{}`requested with no profile using default profile: `{}`, referrer: `{}`", path, proxyConfig.defaultProfile(), request.getHeader(Util.REFERER));
        ...
        if (!profile.equals(possibleShim)) {
            log.warn("Deprecated profile used: `{}`, shimmed to: `{}`, referrer: `{}`", profile, profile, request.getHeader(Util.REFERER));
        }
        if (proxyConfig.acceptedProfiles().contains(possibleShim)) {
            return possibleShim;
        } else {
            log.warn("Profile `{}` was not found in accepted profiles using default: `{}`, referrer: `{}`", possibleShim, proxyConfig.defaultProfile(), request.getHeader(Util.REFERER));
            return proxyConfig.defaultProfile();
        }
    }
